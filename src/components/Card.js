...
export default function Card({ card, onCardClick }) {
  ...
  const likeButtonClassName = (
    `button element__button element__like ${isLiked && 'element__like_active'}`
  );
  ...
  return (
    <div className="element">
      {isOwn && <button className="button element__button element__trash" type="button" aria-label="Удалить место" />}
      <img src={link} alt={`Фото: ${name}`} className="element__image" onClick={handleClick} />
      <div className="element__container">
        <p className="element__title">{name}</p>
        <div className="element__like-container">
          <button className={likeButtonClassName} type="button" aria-label="Поставить лайк" />
